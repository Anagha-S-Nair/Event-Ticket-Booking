...
class RejectedStall extends StatefulWidget {
  @override
  State<RejectedStall> createState() => _RejectedStallState();
}

class _RejectedStallState extends State<RejectedStall> {
  List<Map<String, dynamic>> rejectedstallList = [];
  bool isLoading = true;
  ...
  Future<void> fetchStall() async {
    try {
      final response = await supabase
          ...
          .eq('stallmanager_status', 2);

      print("Fetched Stall Data: $response"); // Debugging output

      setState(() {
        rejectedstallList = List<Map<String, dynamic>>.from(response);
        isLoading = false;
      });
    } catch (e) {
  ...
  Future<void> approveStall(String rid) async {
    try {
      await supabase
          .from('tbl_stallmanager')
          .update({'stallmanager_status': 1}).eq('id', rid);
      fetchStall();
              ...
              child: Container(
                ...
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    rows: rejectedstallList.map((stall) {
